...
Vendored from Nonary/ghcp_proxy (Unlicense).  Local changes: the Windows
reader also replays LevelDB write-ahead logs (a freshly refreshed token lives
there until the next compaction), decodes Chromium's Latin-1/UTF-16
LocalStorage value prefixes, and prefers the latest-expiring token.
"""
...
def _leveldb_log_records(data: bytes):
    """Reassemble the logical records of a LevelDB write-ahead log.

    Checksums are not verified; bounds checks stop at a torn tail record.
    """
    offset = 0
    pending: bytearray | None = None
    while offset + _LEVELDB_LOG_HEADER_SIZE <= len(data):
        block_remaining = _LEVELDB_LOG_BLOCK_SIZE - offset % _LEVELDB_LOG_BLOCK_SIZE
        ...
        start = offset + _LEVELDB_LOG_HEADER_SIZE
        end = start + length
        if record_type == 0 and length == 0:
            # Zero-filled tail of a preallocated block.
            offset += block_remaining
            continue
        if length > block_remaining - _LEVELDB_LOG_HEADER_SIZE or end > len(data):
            return
...
def _leveldb_write_batch_entries(batch: bytes):
    """Yield (key, value) puts from a LevelDB WriteBatch; deletions are skipped."""
    if len(batch) < 12:
        raise ValueError("truncated LevelDB write batch")
    ...
    for _ in range(count):
        ...
        tag = batch[offset]
        offset += 1
        key_length, offset = _decode_leveldb_varint(batch, offset)
        if offset + key_length > len(batch):
            raise ValueError("truncated LevelDB write batch key")
        key = batch[offset : offset + key_length]
        offset += key_length
        if tag == 0:
            continue
        if tag != 1:
            raise ValueError("unknown LevelDB write batch tag")
        value_length, offset = _decode_leveldb_varint(batch, offset)
        if offset + value_length > len(batch):
            raise ValueError("truncated LevelDB write batch value")
        yield key, batch[offset : offset + value_length]
        offset += value_length


def _leveldb_log_entries(path: Path):
    for record in _leveldb_log_records(path.read_bytes()):
        yield from _leveldb_write_batch_entries(record)
...
def _decode_windows_storage_value(value: bytes) -> dict[str, object]:
    # Chromium prefixes LocalStorage values with 0x01 (Latin-1) or 0x00
    # (UTF-16LE); the latter appears once the payload has non-Latin-1 text.
    if value[:1] == b"\x00":
        text = value[1:].decode("utf-16-le")
    elif value[:1] == b"\x01":
        text = value[1:].decode("latin-1")
    else:
...
def load_windows_excel_session(webview_root: Path | None = None) -> dict[str, str]:
    ...
    for database in _windows_leveldb_paths(root):
        sources = [(table, _leveldb_table_entries) for table in database.glob("*.ldb")]
        sources += [(log, _leveldb_log_entries) for log in database.glob("*.log")]
        for source, read_entries in sources:
            try:
                for key, value in read_entries(source):
                    if _STORAGE_KEY.encode("utf-8") not in key:
                        continue
                    headers, expires_at = _cached_session_headers(
                        _decode_windows_storage_value(value)
                    )
                    candidates.append((expires_at, source.stat().st_mtime, headers))
            except (OSError, UnicodeError, ValueError, json.JSONDecodeError) as exc:
                errors.append(f"{source}: {exc}")
    if candidates:
        # Every sign-in issues a token with a fixed lifetime, so the latest
        # expiry is the newest session regardless of which file holds it.
